# DocAI — Plan de Développement Backend + Skills
## 5 Parties · 33 Semaines · 51 Skills · Production Ready

> **Stack :** Java 21 · Spring Boot 4.0.x · Kafka · Keycloak 26 · MongoDB 7 · Valkey 8 · Amazon S3
> **Référence :** DOCAI_BACKEND_MASTER_SPECKIT_F.md v15.0
...
| Feature Flags | Unleash : 6 flags définis (`billing.enabled`, `fraud.v2.enabled`, `extraction.mistral.enabled`, `dashboard.search.enabled`, `notifications.inapp.enabled`, `maintenance.mode`) | `billing.enabled = false` en DEV vérifié |
...
|-------|---------|-------|---------|
| Sécurité core | `TenantJwtFilter`, `GlobalExceptionHandler` RFC 7807, catalogue erreurs, rate limiting Bucket4j, impersonation support | 3j | `GET /v1/documents` sans JWT → HTTP 401 |
...
| `docai-module2-validation` | Luhn, modulo 97, INSEE, BAN, RPPS, ACL, fail-open |
...
| `docai-module3-fraude-revision` | State machine révision, ReviewDecision immuable, SSE alertes < 2s |
...
- [ ] Production Readiness Checklist complète (annexes-standards)

---

---

# Référence — Tous les skills par catégorie

## Skills transversaux (lire en premier)
...
| `docai-observability` | Métriques Micrometer, logs JSON, OpenTelemetry |
...
## Skills adapters patterns
| Skill | Description courte |
|-------|-------------------|
| `docai-adapter-rest` | Controller REST, RFC 7807, JWT, versioning /v1 |
| `docai-adapter-mongodb` | Repository, @Document sans @Indexed, Mongock V001→ |
| `docai-adapter-kafka` | Consumer groups, ResilientKafkaConsumer, Outbox |
| `docai-adapter-valkey` | Cache-Aside, TTL jitter, clés complètes |
| `docai-adapter-s3` | Upload multipart, clé OCR, AbortMultipartUpload |
| `docai-adapter-external-apis` | INSEE, BAN, RPPS, ACL, fail-open, WireMock |
| `docai-kafka-event` | Avro, Apicurio, occurredAt Instant |
| `docai-archunit-verify` | Vérification violations ArchUnit |
| `docai-contract-testing` | Spring Cloud Contract |
| `docai-feature-flag` | Unleash, 6 flags, FeatureFlagPort |
| `docai-email-ses` | Amazon SES, 19 templates Thymeleaf |
| `docai-llm-prompt` | VisionModelAdapter, OCR PDFBox/Tess4J, prompts |

## Skills tests & qualité
| Skill | Description courte |
|-------|-------------------|
| `docai-test-integration` | TestContainers, AbstractIntegrationTest, reuse |
| `docai-bdd-scenario` | Gherkin, Cucumber, step definitions |
| `docai-performance-test` | k6, 3 scénarios, seuils CI |

## Résumé — 51 skills au total

| Catégorie | Nombre |
|-----------|--------|
| Transversaux (architecture, stack, patterns) | 13 |
| Setup & Infrastructure | 3 |
| Commons | 1 |
| Fondations Métier (Module 0) | 7 |
| Pipeline (Modules 1→4) | 13 |
| Produit & Monétisation (Modules 5→7) | 8 |
| Tests & Qualité | 3 |
| Annexes & Standards | 3 |
| **Total** | **51** |
