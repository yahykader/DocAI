...
- [ ] T016 [US3] Validate: run `kubectl apply --dry-run=client -k k8s/overlays/staging` → confirm output shows "deployment.apps/docai-backend configured", "service/docai-backend configured", "horizontalpodautoscaler.autoscaling/docai-backend configured" with 0 errors; confirm `ls k8s/` contains no file named `*secret*` or `*Secret*`
